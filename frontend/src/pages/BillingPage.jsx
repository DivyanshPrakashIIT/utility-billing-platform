...
export default function BillingPage() {
  ...
  const [billForm, setBillForm] = useState({ masterMeterId: '', billingPeriodStart: '', billingPeriodEnd: '', totalAmount: '' })
  const [generateForm, setGenerateForm] = useState({ providerBillId: '', buildingId: '', allocationRuleId: '' })
  ...
  const handleCreateBill = async (e) => {
    ...
    try {
      ...
      setError('Failed to create provider bill. Check master meter ID.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleGenerateBills = async (e) => {
    ...
    setError('')
    try {
      const res = await apiClient.post(`/provider-bills/${generateForm.providerBillId}/generate`, {
        buildingId: Number(generateForm.buildingId),
        allocationRuleId: Number(generateForm.allocationRuleId),
      })
      setGeneratedBills(res.data)
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to generate bills. Check IDs and ensure bill is not already finalized.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow p-6">
          <h3 className="text-lg font-semibold mb-1">Step 1 — Record Provider Bill</h3>
          <p className="text-sm text-gray-500 mb-4">Enter the bill amount received from the utility company.</p>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          <h3 className="text-lg font-semibold mb-1">Step 2 — Generate Unit Bills</h3>
          <p className="text-sm text-gray-500 mb-4">Run the allocation engine to split the bill across all units.</p>
          ...
          {generatedBills.length > 0 && (
            <div className="mt-4 space-y-2">
              <h4 className="font-medium text-gray-700">Generated Bills ({generatedBills.length} units)</h4>
              {generatedBills.map((bill) => (
                <div key={bill.id} className="bg-gray-50 border rounded-lg px-4 py-2 text-sm flex justify-between items-center">
                  <span className="text-gray-500">Unit ID: {bill.unitId}</span>
                  <span className="font-semibold">₹{bill.amountDue}</span>
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${bill.status === 'PAID' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
                    {bill.status}
                  </span>
                </div>
              ))}
