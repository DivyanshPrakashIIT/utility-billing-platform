...
export default function DashboardPage() {
  ...
  const cards = [
    ...
    { title: 'Billing', description: 'Create and manage bills', path: '/billing', color: 'bg-purple-500' },
      ...
      <nav className="bg-white shadow px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm text-gray-600">
            {user?.username} <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-medium">{user?.role}</span>
          </span>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {cards.map((card) => (
            <div
              ...
              className="bg-white rounded-lg shadow p-6 cursor-pointer hover:shadow-md transition border-l-4 border-blue-500"
            >
